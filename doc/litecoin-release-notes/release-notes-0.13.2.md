Aguhcoin Core version 0.13.2 is now available from:

  <https://download.aguhcoin.org/aguhcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/aguhcoin-project/aguhcoin/issues>
...
updates will be released anymore. Without security updates, using a aguhcoin
...
ECDSA signatures inside Aguhcoin transactions now use validation using
...
Previous versions of Aguhcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Aguhcoin Core was to raise the default minimum
relay fee.

Aguhcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Aguhcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running aguhcoind without having to do any manual configuration.
...
Aguhcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Aguhcoin Core will relay transactions with
...
entirely in Aguhcoin Core 0.13, and it is at this time undecided whether the
...
Aguhcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Aguhcoin Core automatically creates a hidden service to listen on, without
manual configuration. Aguhcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Aguhcoin Core is listening, and
...
Aguhcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Aguhcoin
Core to use floating fees. By default, Aguhcoin Core will use floating
fees.
...
(default: `0.0002` AGU/kB).

At all times, Aguhcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) AGU.
Furthermore, Aguhcoin Core will never create transactions paying less than
...
`aguhcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29232 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29232 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /aguhcoinrpc>
        ProxyPass http://127.0.0.1:9232/
        ProxyPassReverse http://127.0.0.1:9232/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass aguhcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< aguhcoinrpc:<password>
    </Location>
...
Restarting aguhcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in aguhcoin.conf
...
aguhcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/aguhcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Aguhcoin Core code base has
...
- `aguhcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `aguhcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Aguhcoin scripting system that in combination with
...
Aguhcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Aguhcoin Core.
...
The two phases are distinct in the Aguhcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Aguhcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Aguhcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Aguhcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aguhcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aguhcoins being spent is exactly
...
- **Increased security for multisig:** Aguhcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Aguhcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Aguhcoin's current ledger (called the UTXO set),
  ...
  note that Aguhcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Aguhcoin users to individually opt-in to almost any change in the Aguhcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Aguhcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Aguhcoin
client to Aguhcoin Core 0.13.2 to signal for segwit support. When a
...
Since Aguhcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `aguhcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aguhcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
