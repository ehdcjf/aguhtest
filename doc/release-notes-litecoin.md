Aguhcoin Core version *0.15.1* is now available from:

  <https://download.aguhcoin.org/aguhcoin-0.15.1/>
...
A number of changes to the way Aguhcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 aguhs per kB (0.00001 AGU). This is to prepare for dropping the minimum transaction fee to 0.1 aguhs per kB (0.0001 AGU) in the next release.
